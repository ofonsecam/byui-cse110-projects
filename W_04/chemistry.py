"""
Project: W04 Chemistry
Author: Oscar Fonseca
Enhancement: I added a dictionary and a function called 'get_formula_name' 
that identifies the common name of a chemical formula (e.g., H2O -> water).
If the formula is found in the dictionary, it prints the name after the molar mass.
"""
...
def make_periodic_table():
    """
    Creates and returns a dictionary with the elements of the periodic table.
    Format: "Symbol": ["Name", Atomic Mass]
    """
    periodic_table_dict = {
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    """
    Calculates and returns the total molar mass of the compound.
    """
    total_molar_mass = 0

    # symbol_quantity_list format: [["Symbol", quantity], ...]
    for symbol, quantity in symbol_quantity_list:
        # Access the atomic mass from the dictionary (index 1)
        element_info = periodic_table_dict[symbol]
        atomic_mass = element_info[1]
        
        # Calculate mass for this element and add to total
...
def get_formula_name(formula):
    """
    Returns the common name of a chemical formula if it exists in the dictionary.
    This function helps exceed the basic requirements.
    """
    known_compounds = {
        "H2O": "water",
        "C6H12O6": "glucose",
...
def main():
    ...
    sample_mass_input = float(input("Enter the mass in grams of the sample: "))

    # 2. Initialize data
    periodic_table = make_periodic_table()

    # 3. Use parse_formula from the provided library
    ...
    number_of_moles = sample_mass_input / molar_mass

    # 5. Output results
    print(f"{molar_mass:.5f} grams/mole")
    print(f"{number_of_moles:.5f} moles")
    
    # Enhancement: Display common name
    common_name = get_formula_name(formula_input)
